...
namespace introspection::details::concepts {
  ...
  concept Bare_character = Bare<T> &&
    (same_as<T, char> || same_as<T, signed char> || same_as<T, unsigned char> ||
     same_as<T, wchar_t> || same_as<T, char8_t> || same_as<T, char16_t> ||
     same_as<T, char32_t>);
      ...
      index_sequence<Index...>)
  {
    return (
      is_member_pointer_v<
        remove_cvref_t<decltype(get<Index>(T::Metadata::member_pointers))>> &&
      ...);
  }
  (make_index_sequence<tuple_size_v<decltype(T::Metadata::member_pointers)>>());
      ...
      index_sequence<Index...>)
  {
    return (
      String_like<
        remove_cvref_t<decltype(get<Index>(T::Metadata::member_names))>> &&
      ...);
  }
  (make_index_sequence<tuple_size_v<decltype(T::Metadata::member_names)>>());
